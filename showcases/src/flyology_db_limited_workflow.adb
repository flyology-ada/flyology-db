...
package body Flyology_DB_Limited_Workflow is
   ...
   procedure Seed_And_Close
     (Context : not null access DB.Storage_Context;
      Timeout : Duration)
   is
      Created        : DB.Database;
   ...
   begin
      ...
      DB.Begin_Transaction (Created, Transaction_ID (4), DB.Snapshot, First_Txn, Result);
      Expect (Result, DB.Success, "first transaction begin failed");
      DB.Put (Created, First_Txn, Accounts, Bytes ("alice"), Bytes ("100"), Result);
      Expect (Result, DB.Success, "first put failed");
      DB.Commit (Created, First_Txn, Timeout, Receipt => Commit_Info, Result => Result);
      Expect (Result, DB.Success, "first commit failed");

      DB.Begin_Transaction (Created, Transaction_ID (5), DB.Snapshot, Second_Txn, Result);
      Expect (Result, DB.Success, "second transaction begin failed");
      DB.Put (Created, Second_Txn, Accounts, Bytes ("bob"), Bytes ("200"), Result);
      Expect (Result, DB.Success, "second put failed");
      DB.Commit (Created, Second_Txn, Timeout, Receipt => Commit_Info, Result => Result);
      ...
      DB.Commit_Group
        (Created, Numbered_ID (13), Group, Timeout, Receipts => Group_Receipts, Result => Result);
      Expect (Result, DB.Success, "atomic group commit failed");

      DB.Begin_Transaction (Created, Transaction_ID (14), DB.Snapshot, Delete_Txn, Result);
      ...
      DB.Commit (Created, Delete_Txn, Timeout, Receipt => Commit_Info, Result => Result);
      Expect (Result, DB.Success, "delete commit failed");

      DB.Begin_Transaction (Created, Transaction_ID (15), DB.Snapshot, Suffix_Txn, Result);
      Expect (Result, DB.Success, "suffix transaction begin failed");
      DB.Put (Created, Suffix_Txn, Accounts, Bytes ("bob"), Bytes ("250"), Result);
      Expect (Result, DB.Success, "suffix account update failed");
      DB.Put (Created, Suffix_Txn, Audit, Bytes ("event-2"), Bytes ("updated"), Result);
      Expect (Result, DB.Success, "suffix audit append failed");
      DB.Commit (Created, Suffix_Txn, Timeout, Receipt => Commit_Info, Result => Result);
      ...
      DB.Begin_Transaction (Created, Transaction_ID (25), DB.Snapshot, Post_Merge_Txn, Result);
      Expect (Result, DB.Success, "post-merge transaction begin failed");
      DB.Put (Created, Post_Merge_Txn, Accounts, Bytes ("bob"), Bytes ("275"), Result);
      Expect (Result, DB.Success, "post-merge account update failed");
      DB.Commit (Created, Post_Merge_Txn, Timeout, Receipt => Commit_Info, Result => Result);
      ...
      DB.Begin_Transaction (Created, Transaction_ID (29), DB.Snapshot, Capacity_Txn, Result);
      Expect (Result, DB.Success, "capacity-edge transaction begin failed");
      DB.Put (Created, Capacity_Txn, Accounts, Bytes ("bob"), Bytes ("300"), Result);
      Expect (Result, DB.Success, "capacity-edge account update failed");
      DB.Commit (Created, Capacity_Txn, Timeout, Receipt => Commit_Info, Result => Result);
