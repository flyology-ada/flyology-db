...
package body Flyology.DB is
   ...
   type Snapshot_Entry_Reference is record
      ...
      Value_Length : Natural := 0;
      Sequence     : Sequence_Number := 0;
      --  Runtime SST-v1 operation tag. Full snapshots use Put; suffix-delta
   ...
   is
      ...
      begin
         ...
         Lease.State.Gate.Copy_Scan_Sources
           (Configuration.ID, Txn.Snapshot_At, Lease.State.Checkpoint_Base, Raw, Captured, Result);
         if Result /= Success then
            Release_Raw;
            return;
         end if;
   ...
   begin
      ...
      Materialize_Physical_Scan_Page (Item, Txn, State, Maximum_Rows, Maximum_Bytes, Rows, Done, Result);
   end Next_Scan_Page;
