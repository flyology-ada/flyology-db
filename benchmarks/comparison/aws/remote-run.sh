...
finish()
{
  ...
  trap - EXIT
  set +e
  if [ -d /mnt/flyology-bench/evidence ]; then
    if ! cp -a /mnt/flyology-bench/evidence "$log_root/evidence" &&
      [ "$status" -eq 0 ]; then
      status=1
    fi
  fi
...
require_file /tmp/flyology-db.bundle
require_file /tmp/flyology-db.patch
require_file /tmp/flyology-db-untracked.tar.gz
require_file /tmp/flyology-db-untracked.json
require_file /tmp/flyology-db-source.env
require_file /tmp/check-power-profile.sh

# shellcheck disable=SC1091
. /tmp/flyology-db-source.env
observed_remote_runner=$(sha256sum /tmp/remote-run.sh | cut -d ' ' -f 1)
[ "$observed_remote_runner" = "$SOURCE_REMOTE_RUNNER_SHA256" ] ||
  fail "remote runner does not match the authenticated source snapshot"
observed_untracked_manifest=$(sha256sum /tmp/flyology-db-untracked.json | cut -d ' ' -f 1)
...
mkdir -p /mnt/flyology-bench/evidence
...
git clone /tmp/flyology-db.bundle /mnt/flyology-bench/flyology-db
repository=/mnt/flyology-bench/flyology-db
git -C "$repository" apply --binary /tmp/flyology-db.patch

observed_untracked=$(sha256sum /tmp/flyology-db-untracked.tar.gz | cut -d ' ' -f 1)
[ "$observed_untracked" = "$SOURCE_UNTRACKED_SHA256" ] ||
  fail "untracked source archive mismatch"
observed_power_detector=$(sha256sum /tmp/check-power-profile.sh | cut -d ' ' -f 1)
[ "$observed_power_detector" = "$SOURCE_POWER_PROFILE_SHA256" ] ||
  fail "power-profile detector mismatch"
tar -C "$repository" -xzf /tmp/flyology-db-untracked.tar.gz
power_detector=$repository/.agents/skills/performance-testing/scripts/check-power-profile.sh
mkdir -p "$(dirname -- "$power_detector")"
install -m 0755 /tmp/check-power-profile.sh "$power_detector"
...
} > /mnt/flyology-bench/evidence/host.env
cp /tmp/flyology-db-source.env /mnt/flyology-bench/evidence/source.env
cp /tmp/flyology-db-untracked.json /mnt/flyology-bench/evidence/untracked-source.json
sha256sum "$power_detector" > /mnt/flyology-bench/evidence/power-profile-detector.sha256
lsblk -O -J > /mnt/flyology-bench/evidence/lsblk.json
lscpu -J > /mnt/flyology-bench/evidence/lscpu.json

./tests/scripts/test.sh
printf '%s\n' 'Flyology.DB AWS deterministic suite passed' |
  tee /mnt/flyology-bench/evidence/deterministic-sentinel.txt
...
post_test_untracked=$(sha256sum /tmp/flyology-db-untracked.tar.gz | cut -d ' ' -f 1)
...
python3 "$repository/benchmarks/comparison/aws/package-source.py" "$repository" \
  /tmp/flyology-db-post-test-untracked.json \
  /tmp/flyology-db-post-test-untracked.tar.gz \
  --allow-manifest /tmp/flyology-db-untracked.json
cmp -s /tmp/flyology-db-untracked.json /tmp/flyology-db-post-test-untracked.json ||
  fail "untracked source changed during tests"
git clone /tmp/flyology-db.bundle "$benchmark_db"
git -C "$benchmark_db" apply --binary /tmp/flyology-db.patch
tar -C "$benchmark_db" -xzf /tmp/flyology-db-untracked.tar.gz
...
install -m 0755 /tmp/check-power-profile.sh "$benchmark_power_detector"
...
mkdir -p /mnt/flyology-bench/tmp
export FLYOLOGY_DB_BENCH_SCRATCH_ROOT=/mnt/flyology-bench/tmp
campaign=/mnt/flyology-bench/evidence/aws-nitro-local.json
python3 "$benchmark/run_realistic_campaign.py" --lane local --output "$campaign"
sha256sum "$campaign" > /mnt/flyology-bench/evidence/aws-nitro-local.sha256
printf '%s\n' 'Flyology.DB AWS Nitro benchmark campaign passed' |
  tee /mnt/flyology-bench/evidence/benchmark-sentinel.txt
...
python3 "$repository/benchmarks/comparison/aws/package-source.py" "$repository" \
  /tmp/flyology-db-final-untracked.json \
  /tmp/flyology-db-final-untracked.tar.gz \
  --allow-manifest /tmp/flyology-db-untracked.json
cmp -s /tmp/flyology-db-untracked.json /tmp/flyology-db-final-untracked.json ||
...
[ -z "$(find /mnt/flyology-bench/tmp -mindepth 1 -print -quit)" ] ||
