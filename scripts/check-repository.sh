...
test -f formal/tla/traces/AggregateCommitCoalescingAuthorityReplay.trace.json
...
# the authenticated live-suffix recovery and cancellation witnesses, and one
# real aggregate authority API recovery replay. Forty-two
# is a fail-closed repository-shape oracle, not model or product policy and not
# a trace, state, or provider limit.
test "$(find formal/tla/traces -type f -name '*.trace.json' | wc -l | tr -d ' ')" -eq 42
