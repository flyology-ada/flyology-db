#!/usr/bin/env python3
"""Validate the three-run LSM compaction trace emitted by TLC."""
...
EXPECTED_ACTIONS = ["Init", "BuildThreeRunMerge", "RecoverMergedRuns"]


def fail(message: str) -> None:
    raise SystemExit(f"invalid three-run LSM compaction witness: {message}")
...
def validate(states: list[dict[str, object]]) -> None:
    ...
    exact_map(final.get("older"), {"K1": "V1"}, "older run")
    exact_map(final.get("first"), {"K1": "V2"}, "first selected run")
    exact_map(final.get("middle"), {"K1": "Tombstone"}, "middle selected run")
    exact_map(final.get("last"), {"K1": "NoMutation"}, "last selected run")
    exact_map(final.get("merged"), {"K1": "Tombstone"}, "merged run")
    exact_map(final.get("newer"), {"K1": "NoMutation"}, "newer run")
    expected_suffix = {"K1": "V1"}
