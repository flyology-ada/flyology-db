...
This guide takes a fresh public clone through one durable Files-backed key. The example creates a database,
writes and commits one value, reads it, publishes an explicit checkpoint with `Flush`, closes every provider
and database owner, reconstructs them, opens the database from object storage, and reads the same value again.
...
`Database_Limits` field, the initial column-family configuration, and each operation timeout. These values are
demonstration geometry, not Flyology.DB defaults or recommendations. A real application must derive and
qualify bounds for its own workload.
...
Create, Commit, and Flush return typed outcomes and receipts. `Success` is conclusive. A definite
pre-admission failure leaves the corresponding caller-owned state reusable according to the operation
...
identity. Retain the exact receipt and invoke the matching read-only reconciliation operation:

- `Resolve_Create` for the same Create receipt;
- `Resolve` for the same Commit receipt;
- `Resolve_Flush` for the same Flush receipt.

The example follows that rule and requires reconciliation to reach a conclusive result before continuing.
Production code must persist enough authority to perform the same exact-receipt reconciliation after
interruption. It must not turn an unknown outcome into a retry with a new idempotency identity.
